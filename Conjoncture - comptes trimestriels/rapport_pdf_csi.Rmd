---
title: "Synthèse des comptes trimestriels : comptes d'agents"
author: '@statjunior'
...
subtitle: Résultats détaillés des *Comptes Nationaux Trimestriels* au 1er trimestre
  2024.
titlepage: yes
...
Pour les entreprises, on s'intéresse principalement aux sociétés non financières (SNF). On présente tout d'abord leur taux de marge et leur taux d'investissement sur longue période. Puis, on décompose les contributions de l'évolution du taux de marge depuis 2019. 

On s'intéresse ensuite aux comptes des administrations publiques (APU). On présente l'évolution des recettes et des dépenses publiques sur longue période ainsi que l'élasticité des recettes publiques par rapport au PIB. On distingue ensuite l'évolution des dépenses par grande composante. Enfin, on présente l'évolution du déficit public sur longue période ainsi que la charge d'intérêt de la dette. 
...
scale_2019t4 <- function(x, na.rm = FALSE) x/x[rdb_menages[,1] == "2019-10-01"]*100
scale_2021t4 <- function(x, na.rm = FALSE) x/x[rdb_menages[,1] == "2021-10-01"]*100
...
liste_t1 <- distinct(csi_2020, TITLE_FR1)
...
rdb_menages <- filter(csi_2020, TITLE_FR1 == "Revenu disponible brut des ménages (y compris les entreprises individuelles)") |>
...
valeur2000 <- rdb_menages_2000 |> filter(DATE == date_max_compte_def) |>
  pull(rdbrut_menages) |> round(digits = 1)
valeur2000 <- valeur2000 - 100

valeurpa2000 <- rdb_menages_2000 |> filter(DATE == date_max_compte_def) |>
...
decompo_rdb_menages <- filter(csi_2020, TITLE_FR1 == "Excédent brut d'exploitation des ménages hors EI (y compris revenus mixtes)" |
...
valeur <- decompo_rdb_menages_def |>
  filter(DATE == date_max_compte_def, Composante == "Revenus d'activité") |>
  pull(value) |>
  round(digits = 1)

valeur2 <- decompo_rdb_menages_def |>
  filter(DATE == date_max_compte_def, Composante == "Prix à la consommation des ménages") |>
  pull(value) |>
  round(digits = 1)
...
```{r,fig.dim = c(14, 9)}
tx_ep_menage <- filter(csi_2020, TITLE_FR1 == "Épargne des ménages (y compris les entreprises individuelles)" |
                                 TITLE_FR1 == "Revenu disponible brut des ménages (y compris les entreprises individuelles)" | 
...
plot_tx_epargne_men <-ggplot(data = tx_ep_menage,
    ...
    labs(title = "Taux d'épargne des ménages", 
         ...
         subtitle = paste0("Lecture: Au ", date_def_litt, ", le taux d'épargne des ménages est de ", valeur <- tx_ep_menage |>filter(DATE == date_max_compte_def) |> pull(tx_epargne) |> round(digits =1)   , " %. Le taux d'épargne financière des ménages est de ",  valeur2 <- tx_ep_menage |>filter(DATE == date_max_compte_def) |> pull(tx_epargne_financiere) |> round(digits =1), " %. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
```{r,fig.dim = c(14, 9)}
compte_snf <- filter(csi_2020, TITLE_FR1 == "Excédent brut d'exploitation des sociétés non financières" |
...
plot_compte_snf <-ggplot(data = compte_snf,
  ...
  labs(title = "Taux de marge et d’investissement des sociétés non financières", 
       ...
       subtitle = paste0("Lecture: Au ", date_def_litt, ", le taux de marge des Sociétés Non Financières (SNF) est de ", valeur <- compte_snf |>filter(DATE == date_max_compte_def) |> pull(tx_marge_snf) |> round(digits =1)   , " %. Le taux d'invesitssement des SNF est de ",  valeur2 <- compte_snf |>filter(DATE == date_max_compte_def) |> pull(tx_fbcf_snf) |> round(digits =1), " %. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
```{r,fig.dim = c(14, 9)}
decompo_tm_snf_2019t4 <- filter(decompo_tm_snf, DATE >as.Date('2019-10-10')) |>
...
valeur <- decompo_tm_snf_2019t4 |>
  filter(DATE == date_max_compte_def, Composante == "Taux de marge") |> pull(value) |> round(digits =1)
valeur2 <- decompo_tm_snf_2019t4 |>
  filter(DATE == date_max_compte_def, Composante == "Masse salariale réelle") |> pull(value) |> round(digits =1)

plot_decompo_tm_snf_2018 <-ggplot(data = decompo_tm_snf_2019t4,
  ...
  labs(title = "Décomposition de la variation du taux de marge des sociétés non financières depuis la fin 2019", 
       
       subtitle = paste0("Lecture: Au ", date_def_litt, ", le taux de marge des SNF ", ifelse(valeur>0, "augmente", "recule"), " de ", valeur, " point depuis le T4-2019. \nLecture: L'évolution de la masse salariale réelle contribue ", ifelse(valeur2 >0, "positivement", "négativement"), " à hauteur de ", valeur2, " points à la variation du taux de marge. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
       
       y = "Variation en points de pourcentage par rapport au 4e trimestre 2019", x = "Facteur de contribution") +
...
```{r, echo=FALSE, message=FALSE, results='hide'}
recette_apu <- filter(csi_2020, TITLE_FR1 == "TVA reçue par les administrations publiques" | 
...
plot_deficit_apu_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Recettes et dépenses des administrations publiques rapportées au PIB",
       ...
       subtitle = paste0("Lecture: Au ", date_def_litt, ", les recettes des APU représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(tot_recettes_apu_pib) |> round(digits =1), " % du PIB et les dépenses des APU représentent ", valeur2 <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(tot_depenses_apu_pib) |> round(digits =1), " % du PIB.\nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
valeur <- structure_po2 %>%
  filter(DATE == date_max_compte_def, title == "Impôts nets de subvention sur la production") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)

valeur2 <- structure_po2 %>%
  filter(DATE == date_max_compte_def, title == "Ensemble des prélèvements obligatoires") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)
...
valeur <- structure_po %>%
  filter(DATE == date_max_compte_def, title == "Impôts nets de subventions sur la production") %>%
  pull(percent)  %>%
  round(digits = 1)
...
valeur1 <- elasticite_po1 %>%
  filter(DATE == date_max_compte_def, title == "Glissement annuel des recettes des APU") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)

valeur11 <- elasticite_po1 %>%
  filter(DATE == date_max_compte_def, title == "Glissement annuel du PIB valeur") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)
...
valeur2 <- elasticite_po2 %>%
  filter(DATE == date_max_compte_def, title == "Elasticité des recettes") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)
...
valeur <- dynamisme_recettes %>%
  filter(DATE == date_max_compte_def, title == "Recettes totales") %>%
  pull(OBS_VALUE)  %>%
  round(digits = 1)
...
plot_dep_fonctionnement_apu_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Total des dépenses de fonctionnement", 
       subtitle = paste0("Lecture: Au ", date_def_litt, ", les dépenses de fonctionnement représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_fonctionnement_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"), 
       y = "% du PIB valeur", x = "Année") +
  scale_x_date(limits = as.Date(c("1980-01-01", NA))) + 
...
plot_subventions_apu_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Subventions versées par les administrations publiques", 
              subtitle = paste0("Lecture: Au ", date_def_litt, ", les subventions représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_subventions_apu_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"),
       y = "% du PIB valeur", x = "Année") +
  scale_x_date(limits = as.Date(c("1980-01-01", NA))) + 
...
plot_interet_verses_apu_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Charges d'intérêts versées par les administrations publiques", 
       subtitle = paste0("Lecture: Au ", date_def_litt, ", la charge d'intérêt représente ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_interet_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"),
...
plot_transferts_sociaux_nature_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Transferts sociaux et apparentés (en espèce ou en nature)", 
       subtitle = paste0("Lecture: Au ", date_def_litt, ", les transferts sociaux représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_transferts_sociaux_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"),
       y = "% du PIB valeur", x = "Année") +
  scale_x_date(limits = as.Date(c("1980-01-01", NA))) + 
...
plot_autres_transferts_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Transferts entre administrations", 
        subtitle = paste0("Lecture: Au ", date_def_litt, ", les transferts entre administrations représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_autres_transferts_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"),
...
plot_investissement_apu_pib <- ggplot(data = recette_apu,
  ...
  labs(title = "Dépenses reliées à l'investissement", 
               ...
               subtitle = paste0("Lecture: Au ", date_def_litt, ", les dépenses d'investissement représentent ", valeur <- recette_apu |> filter(DATE == date_max_compte_def) |> pull(depenses_investissement_pib) |> round(digits =1), " % du PIB. \nSource : Insee (Comptes nationaux Trimestriels - Dernier point : ", date_max_compte_def ,") \nCalculs et graphiques : @statjunior"),
...
```{r,fig.dim = c(14, 9)}
#######Déficit des APU
deficit_apu <- filter(csi_2020, TITLE_FR1 == "Capacité (+) ou besoin (-) de financement des administrations publiques" | 
...
plot_deficit_apu_pib <- ggplot(data = deficit_apu,
  ...
  labs(title = "Déficit public des APU et déficit primaire hors charge d'intérêt rapporté au PIB", 
        ...
        subtitle = paste0("Lecture: Au ", date_def_litt, ", le déficit public est de ", valeur <-deficit_apu |>filter(DATE == date_max_compte_def) |> pull(def_total_pib) |> round(digits =1), " % du PIB. Le déficit primaire (hors charge d'intérêt) est de ", valeur2 <- deficit_apu |>filter(DATE == date_max_compte_def) |> pull(def_primaire_pib) |> round(digits =1), " % du PIB. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
```{r,fig.dim = c(14, 16)}
interet_apu <- filter(csi_2020, TITLE_FR1 == "Intérêts reçus par les administrations publiques" | 
                       TITLE_FR1 =="Intérêts versés par les administrations publiques") |>
  mutate(TITLE_FR1 = case_when(
    TITLE_FR1 == "Intérêts reçus par les administrations publiques"~"interet_recu_apu", 
    TITLE_FR1 == "Intérêts versés par les administrations publiques"~"interet_verses_apu"
...
plot_interet_val <- ggplot(data = interet_apu,
  ...
  labs(title = "Charge d'intérêt nette nominale des administrations publiques", 
        ...
        subtitle = paste0("Lecture: Au ", date_def_litt, ", la charge d'intérêt nette des APU est de ",valeur <- interet_apu |> filter(DATE == date_max_compte_def) |> pull(interet_net) |> round(digits =1), " milliards d'euros. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
plot_interet_pib <- ggplot(data = interet_apu,
  ...
  labs(title = "Charge d'intérêt nette des administrations publiques en proportion du PIB nominal", 
      ...
      subtitle = paste0("Lecture: Au ", date_def_litt, ", la charge d'intérêt nette des APU représente ",valeur <- interet_apu |> filter(DATE == date_max_compte_def) |> pull(interet_pib) |> round(digits =1), " % du PIB nominal. \nSource : Insee - Comptes nationaux trimestriels (Résultats détaillés du ", date_def_litt,"). \nCalculs et graphiques : @statjunior"),
...
```{r,fig.dim = c(14, 9)}
capbesfi <- filter(csi_2020, TITLE_FR1 == "Capacité (+) ou besoin (-) de financement du reste du monde" |
...
valeur <- capbesfi |> filter(DATE == date_max_compte_def) |> pull(capbesfi_total_pib) |>
  round(digits = 1)
valeur2 <- capbesfi |> filter(DATE == date_max_compte_def, Composante == "Sociétés non financières") |> pull(value) |>
  round(digits = 1)
valeur3 <- capbesfi |> filter(DATE == date_max_compte_def, Composante == "Ménages") |> pull(value) |>
  round(digits = 1)
valeur4 <- capbesfi |> filter(DATE == date_max_compte_def, Composante == "APU") |> pull(value) |>
  round(digits = 1)
