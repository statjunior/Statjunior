---
title: "Inflation : perspective nationale et comparaison européenne"
subtitle: "Indice des prix à la consommation de l'Insee et Indice des Prix à la Consommation Harmonisée d'Eurostat"
author: "@statjunior"
...
Ce rapport **Rmarkdwon** sur l'inflation synthétise les publications de l'Indice des Prix à la Consommation de l'Insee. Le dernier point connu est `r point_max_litt`.
...
Le code source permettant de générer ce document est disponible sur Git [en cliquant ici](https://github.com/statjunior/Statjunior/tree/main/Inflation/). 
...
plot_contrib_ipc <-  ggplot(data=gliss_composante_ipc , aes(x=DATE, y= contrib_gliss_ipc, fill = forcats::fct_rev(Composante))) +
  ...
  labs(title = "Contribution des postes de consommation au glissement annuel de l'IPC", 
       ...
       subtitle = paste0("Lecture : En ", point_max_litt, ", l'inflation est de 4,8 %. L'alimentation contribue pour 1,8 point à l'inflation. \nSource : Insee (Indice des Prix à la Consommation - dernier point :", point_max_litt,") \nCalculs et graphiques : @statjunior"), 
...
valeur <- analyse_ipc2 |> filter(DATE == max(DATE), Composante == "Alimentation") %>%  pull(value) %>%  round(digits = 1)
valeur <- valeur - 100 
plot_ipc2 <- ggplot(data=analyse_ipc2, aes(x=DATE, y=value, fill = Composante)) +
  ...
  labs(title = "Evolution des composantes de l'inflation depuis juin 2021",
       ...
       subtitle = paste("Lecture : En", point_max_litt, ", les prix alimentaires ont augmenté de 11 % sur un an et de 0,3 % sur un mois. Les prix énergétiques progressent de 7 % sur un an et de 7 % sur un mois. \nSource : Insee (Enquêtes de conjoncture, données CVS - dernier point :", point_max_litt,") \nCalculs et graphiques : @statjunior"), 
...
analyse_ipc3 <- analyse_ipc3 |>
  ...
  mutate(gliss_annuel = value/lag(value, 12)-1) |>
  mutate(gliss_mensuel = value/lag(value, 1)-1) |>
  mutate(gliss_2021 = value/value[DATE == "2021-06-01"]-1) |>
...
prepare_plot_ipc3 <- filter(analyse_ipc3, !is.na(value_max_date)) |>
  ...
  mutate(gliss_2021 = gliss_2021*100) |>
...
## Evolution des prix alimentaires depuis juin 2021
```{r,fig.dim = c(14, 8)}
###Prix alimentaires en évolution annuelle
plot_ipc3_2021 <-ggplot(data = prepare_plot_ipc3,
                           aes(x = gliss_2021,
                               y = reorder(Composante,gliss_2021), 
                               fill =  date))+
  geom_col( position = "dodge2") +
  labs(title = "Glissement annuel des prix alimentaires en avril 2023", 
       subtitle = "Lecture : Au mois d'avril 2023, les prix des produits alimentaires et boissons non alcoolisées ont augmenté de X % par rapport à avril 2022. \nSource : Insee (Indice des prix à la consommation) \nCalculs et graphiques : @statjunior", 
...
plot_ipc4_annuel <-ggplot(data = prepare_plot_ipc4,
  ...
  geom_col(color = "#00B8E7", fill="#00B8E7") +
  labs(title = "Glissement annuel des prix énergétiques", 
...
## Evolution des prix énergétiques depuis 2021
```{r,fig.dim = c(14, 8)}
plot_ipc4_2021 <-ggplot(data = prepare_plot_ipc4,
                          aes(x = gliss_2021,
                              y = reorder(Composante,gliss_2021))) +
  geom_col(color = "#00B8E7", fill="#00B8E7") +
  labs(title = "Evolution des prix énergétiques depuis juin 2021", 
       subtitle = "Lecture : Au mois d'avril 2023, les prix de l'électricité ont augmenté de X % par rapport à avril 2022. \nSource : Insee (Indice des prix à la consommation) \nCalculs et graphiques : @statjunior", 
       y = "Type de produit énergétique", x = "Pourcentage (%)") +
  theme_ipsum( plot_margin = margin(0, 0, 0, 0),
               plot_title_size = 18,
