...
#Chemin d'accès
setwd <- setwd("C:/Users/gasto/Documents/GitHub/Statjunior/Demandeurs d'emploi (Pôle Emploi)/")
getwd()
...
Le code source permettant de générer ce document est disponible sur Git [en cliquant ici](https://github.com/statjunior/Statjunior/tree/main/Demandeurs%20d'emploi%20(P%C3%B4le%20Emploi)/).
...
## Catégorie A et Catégorie A-B-C (France hors Mayotte)
```{r,fig.dim = c(14, 9)}
plot_abc <-ggplot(data = df_abc,
                               aes(x = DATE,
                                   y = OBS_VALUE, fill=forcats::fct_relevel(TITLE_FR2, c("Catégorie A", "Catégories A, B et C"), after = 1))) +
  geom_line(aes(col = TITLE_FR2), size =1.1) +
  labs(title = "Demandeurs d'emploi tenus de chercher un emploi depuis 1996", 
...
```{r, echo=FALSE, message=FALSE, results='hide'}
var_pe <- rbind(df_abc, df_abcde) |>
  filter(TITLE_FR4 == "France hors Mayotte" | (TITLE_FR4 == "France métropolitaine" & TITLE_FR2 == "Ensemble des catégories")) |>
  group_by(TITLE_FR2) |>
  arrange(DATE) |>
  mutate(gliss_annuel = ((OBS_VALUE/lag(OBS_VALUE, 4))-1)*100) |>
  mutate(var_trim = ((OBS_VALUE/lag(OBS_VALUE, 1))-1)*100) |>
  ungroup() |>
  pivot_longer(cols=c(5:6), names_to = "Variation", values_to = "value2") |>
  mutate(Variation = ifelse(Variation == "gliss_annuel", "Glissement annuel", "Variation trimestrielle"))

valeur <- var_pe %>% filter(DATE == max(DATE), TITLE_FR2 == "Catégorie A", Variation == "Variation trimestrielle") %>% pull(value2) |> round(digits = 1)
valeur2 <- var_pe %>% filter(DATE == max(DATE), TITLE_FR2 == "Catégorie A", Variation == "Glissement annuel") %>% pull(value2) |> round(digits = 1)
...
plot_var_pe <-ggplot(data = var_pe,
  ...
  labs(title = "Variation trimestrielle et annuelle des demandeurs d'emplois", 
       ...
       subtitle = paste0("Lecture : Au ",date_prov_litt, ", le nombre de demandeurs d'emplois en catégorie A ", ifelse(valeur>0, "augmente", "baisse"),  " de ", valeur,  " % par rapport au dernier trimestre et ", ifelse(valeur2>0, "augmente", "baisse")," de ", valeur2, " % sur un an. \nChamp : France hors Mayotte (cat A, ABC) et France métropolitaine (Toutes catégories) \nSource : Pôle Emploi-Dares (Données CVS-CJO ; Dernier point : ", date_prov_litt, ") \nCalculs et graphiques : @statjunior"), 
...
plot_insee_dares <-ggplot(data = pole_emploi_insee,
  ...
  labs(title = "Ecart entre les demandeurs d'emploi de Pôle Emploi et le chômage mesuré par l'Insee", 
       ...
       subtitle = paste0("Lecture : Au ",date_prov_litt, ", l'écart entre le nombre de catégories A mesurées par Pôle Emploi et le nombre de chômeur BIT mesuré par l'Insee est de ", valeur <- pole_emploi_insee %>% filter(DATE == max(DATE)) %>% pull(ecart_cata_bit) %>%  round(digits =1), " million d'individus. \nChamp : France métropolitaine \nSource : Pôle Emploi-Dares (Données CVS-CJO ; Dernier point : ", date_prov_litt, ") \nCalculs et graphiques : @statjunior"), 
